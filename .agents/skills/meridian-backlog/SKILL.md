...
2. Identify the GitHub owner from Meridian's `origin` and the authenticated account. List the owner's repositories with pagination; include `meridian` and every owned repository whose name contains `scrub` (including `hudscrub`). Do not use a fixed five-repo list: discover newly created, renamed, private, and archived repos, and record any inaccessible repo as a coverage gap. Check explicitly named additional owners too.
