...
const { REPLAY_PROVENANCE_NOTE, SCRATCHPAD_COUNTER_INSTRUCTION, buildNoClientCwdNote } = await import("../proxy/query")
...
describe("POST /v1/chat/completions — non-streaming", () => {
  ...
  it("keeps client instructions intact with transport provenance and no claude_code preset", async () => {
    ...
    expect(capturedOptions?.systemPrompt).toBe("You are TestBot. Reply with exactly: ZEBRA-7" + buildNoClientCwdNote(process.cwd(), true) + REPLAY_PROVENANCE_NOTE + SCRATCHPAD_COUNTER_INSTRUCTION)
  })
...
describe("POST /v1/chat/completions — Jcode session continuity", () => {
  ...
  it("resumes the same SDK session for two turns with one verified Jcode key", async () => {
    ...
    expect(capturedOptionHistory[1]?.systemPrompt).toBe("stable system" + buildNoClientCwdNote(process.cwd(), true) + REPLAY_PROVENANCE_NOTE + SCRATCHPAD_COUNTER_INSTRUCTION)
  })
