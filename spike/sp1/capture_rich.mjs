...
async function main() {
  const child = spawn("node", [join(process.cwd(), "spike/sp1/acp_agent.mjs")], { stdio: ["pipe", "pipe", "inherit"], env: { ...process.env, CLAUDE_PTY_OUTPUT_MODE: "rich" } });
  const conn = new ClientSideConnection(() => new C, ndJsonStream(Writable.toWeb(child.stdin), Readable.toWeb(child.stdout)));
  const cwd = mkdtempSync(join(tmpdir(), "sp1-rich-"));
  await conn.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  const ns = await conn.newSession({ cwd, mcpServers: [] });
  msg = [];
  const r = await conn.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: "Output EXACTLY this markdown: ## Heading\\nNormal **bold** and `inline` and a [link](https://example.com).\\n- one\\n- two\\n\\n| A | B |\\n|---|---|\\n| 1 | 2 |" }] });
  console.log("stopReason:", r.stopReason, `
=== RICH reconstructed markdown (message channel) ===
` + msg.join(""));
  child.kill();
  process.exit(0);
}
