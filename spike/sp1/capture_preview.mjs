...
async function main() {
  const child = spawn("node", [join(process.cwd(), "spike/sp1/acp_agent.mjs")], {
    stdio: ["pipe", "pipe", "inherit"],
    env: { ...process.env, CLAUDE_PTY_OUTPUT_MODE: "preview" }
  });
  const conn = new ClientSideConnection(() => new C, ndJsonStream(Writable.toWeb(child.stdin), Readable.toWeb(child.stdout)));
  const cwd = mkdtempSync(join(tmpdir(), "sp1-pv-"));
  await conn.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  const ns = await conn.newSession({ cwd, mcpServers: [] });
  msg = [];
  thought = [];
  const r = await conn.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: "Show me a TypeScript debounce function in a fenced code block. Just the code." }] });
  console.log("stopReason:", r.stopReason);
  console.log(`
=== THOUGHT channel (live PTY preview — expect lossy/flattened code) ===`);
  console.log(thought.join(""));
  console.log("\n=== MESSAGE channel (final — expect perfect ```fences) ===");
