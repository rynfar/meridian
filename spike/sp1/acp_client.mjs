...
async function main() {
  const agentPath = join(process.cwd(), "spike/sp1/acp_agent.mjs");
  ...
  const conn = new ClientSideConnection(() => new TestClient, ndJsonStream(toAgent, fromAgent));
  const cwd = mkdtempSync(join(tmpdir(), "sp1-acplib-"));
  console.log("[client] initialize →", JSON.stringify(await conn.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })));
  const ns = await conn.newSession({ cwd, mcpServers: [] });
  console.log("[client] newSession →", JSON.stringify(ns));
  const sessionId = ns.sessionId;
  const r3 = await conn.prompt({
    sessionId,
    prompt: [{ type: "text", text: "Remember this number: 73. Reply with exactly: OK" }]
  });
  console.log("[client] prompt1 →", JSON.stringify(r3), "| streamed:", JSON.stringify(chunks.join("")));
  const r4 = await conn.prompt({
    sessionId,
    prompt: [{ type: "text", text: "What number did I say? Reply with just the number." }]
  });
  console.log("[client] prompt2 →", JSON.stringify(r4));
  const streamed = chunks.join(" ");
  console.log("[client] all agent_message_chunk text:", JSON.stringify(streamed));
  const ok = r3.stopReason === "end_turn" && r4.stopReason === "end_turn" && streamed.includes("73");
  console.log(ok ? "ACP_LIB: PASS (reference client ↔ reference-built agent, spec-compliant)" : "ACP_LIB: FAIL");
  child.kill();
  process.exit(ok ? 0 : 1);
}
main();
