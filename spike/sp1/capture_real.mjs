...
// spike/sp1/capture_real.ts
import { spawn } from "node:child_process";
import { Readable, Writable } from "node:stream";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { mkdtempSync, rmSync, existsSync, writeFileSync } from "node:fs";
var DUMP = "/tmp/claude-real-capture.txt";
var active = [];
...
async function main() {
  ...
  const child = spawn("node", [agentPath], {
    ...
    env: { ...process.env, CLAUDE_PTY_DUMP: DUMP }
  });
  const toAgent = Writable.toWeb(child.stdin);
  const fromAgent = Readable.toWeb(child.stdout);
  const conn = new ClientSideConnection(() => new CaptureClient, ndJsonStream(toAgent, fromAgent));
  const cwd = mkdtempSync(join(tmpdir(), "sp1-capture-"));
  ...
  const PROMPTS = [
    "List the first 5 prime numbers. Format as a numbered markdown list, one per line, each with a one-sentence note.",
    "Give me a markdown table with 3 rows comparing TCP vs UDP (columns: Feature, TCP, UDP).",
    "Is 1009 a prime number? Think it through step by step, then give a one-line final answer."
  ];
  for (let i = 0;i < PROMPTS.length; i++) {
    active = [];
    const r = await conn.prompt({ sessionId, prompt: [{ type: "text", text: PROMPTS[i] }] });
    const streamed = active.join("");
    console.log(`
===== TURN ${i + 1} =====`);
    console.log("prompt:", PROMPTS[i]);
    console.log("stopReason:", r.stopReason);
    console.log("ACP-streamed text (what the extractor produced):");
    console.log(JSON.stringify(streamed));
  }
  console.log(`
[capture] raw PTY dump written to ${DUMP}`);
  child.kill();
  process.exit(0);
}
main();
