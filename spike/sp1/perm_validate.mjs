...
// spike/sp1/perm_validate.run.ts
import { homedir, tmpdir } from "node:os";
import { join as join2 } from "node:path";
import { mkdtempSync, existsSync as existsSync2 } from "node:fs";
async function main() {
  const claudePath = join2(homedir(), ".local/bin/claude");
  const cwd = mkdtempSync(join2(tmpdir(), "sp1-perm-"));
  const sess = new ClaudeSession(claudePath, cwd, ["--permission-mode", "default"]);
  const dialogs = [];
  try {
    const ready = await sess.ready();
    console.log("ready:", ready);
    const r = await sess.turn("Use the Write tool to create a file ./out.txt with the exact contents: PERMOK-99", undefined, async (p) => {
      dialogs.push(p.options.map((o) => o.name).join(" | "));
      return p.defaultIndex;
    });
    const fileMade = existsSync2(join2(cwd, "out.txt"));
    console.log("dialogs seen:", JSON.stringify(dialogs));
    console.log("turn done:", r.doneDetected, "latency:", r.latencyMs, "text:", JSON.stringify(r.text));
    console.log("out.txt created:", fileMade);
    const ok = dialogs.length >= 1 && r.doneDetected && fileMade;
    console.log(ok ? "PERM_VALIDATE: PASS" : "PERM_VALIDATE: FAIL");
    process.exit(ok ? 0 : 1);
  } finally {
    await sess.close();
  }
