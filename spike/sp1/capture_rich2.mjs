...
async function main() {
  ...
  const cwd = mkdtempSync(join(tmpdir(), "sp1-rich2-"));
  await conn.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  const ns = await conn.newSession({ cwd, mcpServers: [] });
  msg = [];
  const r = await conn.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: "Output EXACTLY: Here is a function:\\n```js\\nfunction add(a, b) {\\n  return a + b;\\n}\\n```\\nThat **adds** two numbers." }] });
  console.log("stopReason:", r.stopReason, `
=== RICH (with code fence) ===
