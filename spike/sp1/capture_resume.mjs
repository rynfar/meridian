...
// spike/sp1/capture_resume.ts
import { spawn } from "node:child_process";
import { Readable, Writable } from "node:stream";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { mkdtempSync } from "node:fs";
var agentPath = join(process.cwd(), "spike/sp1/acp_agent.mjs");
function connect(child, onUpdate) {
  ...
  return new ClientSideConnection(() => new C, ndJsonStream(Writable.toWeb(child.stdin), Readable.toWeb(child.stdout)));
}
var text = (u) => {
  const x = u.update;
  if (x.sessionUpdate === "agent_message_chunk" && x.content.type === "text")
    return "A: " + x.content.text;
  if (x.sessionUpdate === "user_message_chunk" && x.content.type === "text")
    return "U: " + x.content.text.slice(0, 50);
  if (x.sessionUpdate === "tool_call")
    return "tool_call: " + x.title;
  if (x.sessionUpdate === "tool_call_update")
    return "tool_update";
  return "";
};
async function main() {
  const cwd = mkdtempSync(join(tmpdir(), "sp1-rs-"));
  const p1 = spawn("node", [agentPath], { stdio: ["pipe", "pipe", "inherit"] });
  const c1 = connect(p1, () => {});
  await c1.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  const ns = await c1.newSession({ cwd, mcpServers: [] });
  console.log("newSession id:", ns.sessionId);
  await c1.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: "Remember: my favorite number is 4242. Reply OK." }] });
  console.log("P1 taught the fact; closing agent.");
  p1.kill();
  await new Promise((r2) => setTimeout(r2, 1500));
  const replay = [];
  const p2 = spawn("node", [agentPath], { stdio: ["pipe", "pipe", "inherit"] });
  const c2 = connect(p2, (u) => {
    const t = text(u);
    if (t)
      replay.push(t);
  });
  const init = await c2.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  console.log("loadSession capability:", init.agentCapabilities?.loadSession);
  await c2.loadSession({ sessionId: ns.sessionId, cwd, mcpServers: [] });
  console.log(`
--- replayed history (` + replay.length + " updates) ---");
  for (const r2 of replay.slice(0, 8))
    console.log("  " + r2);
  const live = [];
  replay.length = 0;
  const r = await c2.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: "What's my favorite number? Reply with just the number." }] });
  const answer = replay.filter((x) => x.startsWith("A:")).join(" ");
  console.log(`
follow-up stopReason:`, r.stopReason);
  console.log("follow-up answer:", JSON.stringify(answer));
  console.log("CONTEXT RESTORED (contains 4242):", /4242/.test(answer));
  p2.kill();
  process.exit(0);
}
main();
