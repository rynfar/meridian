...
// spike/sp1/mcp_bridge.run.ts
import { homedir, tmpdir } from "node:os";
import { join as join2 } from "node:path";
import { mkdtempSync, writeFileSync } from "node:fs";
async function main() {
  const claudePath = join2(homedir(), ".local/bin/claude");
  const serverPath = join2(process.cwd(), "spike/sp1/mcp_proof_server.py");
  const cwd = mkdtempSync(join2(tmpdir(), "sp1-mcp-"));
  const cfgPath = join2(cwd, "mcp-proof.json");
  writeFileSync(cfgPath, JSON.stringify({ mcpServers: { proof: { command: "python3", args: [serverPath] } } }));
  const sess = new ClaudeSession(claudePath, cwd, [
    ...
    cfgPath,
    "--allowedTools",
    "mcp__proof__get_build_code"
  ]);
  const dialogs = [];
  try {
    const ready = await sess.ready();
    console.log("ready:", ready);
    const r = await sess.turn("Use the get_build_code tool to retrieve the build code, then reply with exactly: BUILD=<the code it returned>", undefined, async (p) => {
      dialogs.push(p.options.map((o) => o.name).join(" | "));
      return p.defaultIndex;
    });
    console.log("permission dialogs:", JSON.stringify(dialogs));
    console.log("done:", r.doneDetected, "| response:", JSON.stringify(r.text));
    const usedResult = r.text.includes("BUILDCODE-SP1-7Q9Z");
    console.log(usedResult ? "MCP_BRIDGE: PASS" : "MCP_BRIDGE: FAIL (code not in response)");
    process.exit(usedResult ? 0 : 1);
  } finally {
    await sess.close();
  }
}
