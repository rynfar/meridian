...
class C {
  async sessionUpdate(p) {
    ...
    if (u.sessionUpdate === "agent_message_chunk" && u.content.type === "text")
      ...
      events.push(`THOUGHT: ${u.content.text.slice(0, 60)}`);
    else if (u.sessionUpdate === "tool_call")
      events.push(`TOOL_CALL  [${u.kind}] title="${u.title}" status=${u.status}`);
    else if (u.sessionUpdate === "tool_call_update") {
      const c = u.content?.[0];
      const out = c && c.type === "content" && c.content.type === "text" ? c.content.text.replace(/\n/g, " ").slice(0, 80) : "(none)";
      events.push(`TOOL_UPDATE status=${u.status} output="${out}"`);
    }
  }
...
async function main() {
  ...
  const conn = new ClientSideConnection(() => new C, ndJsonStream(Writable.toWeb(child.stdin), Readable.toWeb(child.stdout)));
  const cwd = mkdtempSync(join(tmpdir(), "sp1-clean-"));
  await conn.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} });
  const ns = await conn.newSession({ cwd, mcpServers: [] });
  const PROMPTS = [
    "Run the bash command `echo hello from pylon` and tell me what it printed."
  ];
  for (let i = 0;i < PROMPTS.length; i++) {
    chunks = [];
    events = [];
    const r = await conn.prompt({ sessionId: ns.sessionId, prompt: [{ type: "text", text: PROMPTS[i] }] });
    console.log(`
===== TURN ${i + 1} (${r.stopReason}) =====`);
    for (const e of events)
      console.log("  " + e);
    console.log("TEXT:", chunks.join(""));
  }
  child.kill();
  process.exit(0);
}
