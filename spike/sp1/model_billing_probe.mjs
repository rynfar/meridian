// spike/sp1/model_billing_probe.ts
...
function decideOutcome(text, turnEnded, parked) {
  if (parked !== null) {
    return { kind: "tool_use", text, id: parked.id, name: parked.name, input: parked.input };
  }
  if (turnEnded)
    return { kind: "text", text };
  return null;
}
function rebuildOutcome(outcome, fullText) {
  return outcome.kind === "tool_use" ? { ...outcome, text: fullText } : { kind: "text", text: fullText };
}
...
`);
  ...
  async bridgedTurn(prompt, onUpdate) {
    const t0 = Date.now();
    this.answeredSig = "";
    this.preTurnBullets = countBullets(await this.screen.rows());
    this.tail.markBaseline();
    ...
    return this.pollUntilOutcome(t0, onUpdate);
  }
  async resumeBridged(toolUseId, resultText, onUpdate) {
    const t0 = Date.now();
    this.bridge.resume(toolUseId, resultText);
    return this.pollUntilOutcome(t0, onUpdate);
  }
  async pollUntilOutcome(t0, onUpdate) {
    let fullText = "";
    let turnEnded = false;
    const emit = (events) => {
      for (const ev of events) {
...
` : "") + ev.text;
    ...
    const failsafe = Date.now() + TURN_FAILSAFE_MS;
    while (Date.now() < failsafe) {
      emit(this.tail.read());
      const outcome = decideOutcome(fullText, turnEnded, this.bridge?.peekParkedCall() ?? null);
      if (outcome !== null) {
        emit(this.tail.read());
        if (DEBUG) {
          process.stderr.write(`[dbg ${Date.now() - t0}ms] BRIDGED outcome=${outcome.kind} textLen=${fullText.length}
`);
        }
        return rebuildOutcome(outcome, fullText);
      }
      if (this.exited || Date.now() - this.lastByteAt > SILENCE_LIMIT_MS)
        break;
      await sleep(50);
...
// spike/sp1/model_billing_probe.ts
var claudePath = join3(homedir2(), ".local/bin/claude");
var WORK_CONFIG = "/Users/rynfar/.claude-test-work";
var MODELS = process.argv.slice(2).length ? process.argv.slice(2) : ["", "claude-opus-4-8", "claude-sonnet-4-6"];
function classify(text) {
  const t = (text || "").toLowerCase();
  if (t.includes("out of extra usage") || t.includes("extra usage") && t.includes("1m"))
    return "EXTRA_USAGE";
  if (t.includes("api error") || t.includes("invalid_request"))
    return "API_ERROR";
  if (t.includes("probe-ok"))
    return "OK";
  return "OTHER";
}
async function probe(model) {
  const cwd = mkdtempSync(join3(tmpdir(), "sp1-modelprobe-"));
  const args = model ? ["--model", model] : [];
  const label = model || "(default, no --model)";
  const sess = new ClaudeSession(claudePath, cwd, args, {
    mode: "clean",
    env: { CLAUDE_CONFIG_DIR: WORK_CONFIG }
  });
  try {
    const ready = await Promise.race([
      sess.ready(),
      new Promise((r) => setTimeout(() => r("READY_TIMEOUT"), 45000))
    ]);
    if (ready === "READY_TIMEOUT") {
      console.log(`[${label}] READY_TIMEOUT`);
      return;
    }
    const tr = await Promise.race([
      sess.turn("Reply with exactly: PROBE-OK"),
      new Promise((r) => setTimeout(() => r({ text: "<<TURN TIMEOUT 60s>>" }), 60000))
    ]);
    const verdict = classify(tr.text);
    console.log(`[${label}] => ${verdict} :: ${JSON.stringify((tr.text || "").slice(0, 160))}`);
  } catch (e) {
...
async function main() {
  console.log(`probing work account (${WORK_CONFIG}) over interactive PTY...`);
