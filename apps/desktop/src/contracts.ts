...
export type Action = 'test-notification' | 'refresh' | 'check-updates' | 'install' | 'activate' | 'start' | 'stop' | 'restart' | 'save-preferences' | 'switch-profile' | 'add-profile' | 'login-profile' | 'login-code' | 'reload-plugins' | 'set-features' | 'open-page' | 'release-notes' | 'export-diagnostics' | 'acknowledge' | 'login-at-startup' | 'take-ownership' | 'return-headless' | 'open-login'
