...
| Letta Code | ✅ Verified | `/v1/chat/completions` (see [Agent Setup](#letta-code)) — adapter reads the `conv-<uuid>` id from Letta's own agent-info reminder as the session key, so history is preserved and the prompt cache reused across turns (on turn 2 of a two-turn Sonnet session, 15,300 of 15,365 prompt tokens were read from cache and only 63 written, against zero cache reuse for the same body without the reminder) |
